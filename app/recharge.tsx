...
export default function Recharge({ need, balance, upi, onClose, onCredited }: { need: number; balance: number; upi: { id: string; name: string }; onClose: () => void; onCredited?: (balancePaise: number) => void }) {
  const [amount, setAmount] = useState<number>(RECHARGE_OPTIONS_INR[0])
  const [qr, setQr] = useState<string>('')
  const [utr, setUtr] = useState('')
  const [state, setState] = useState<'pay' | 'sent' | 'credited' | 'error'>('pay')
  ...
  async function submit() {
    ...
    const j = (await res.json()) as { ok: boolean; message?: string; data?: { status?: 'approved' | 'pending'; balance_paise?: number } }
    if (j.ok && j.data?.status === 'approved') {
      setState('credited')
      if (typeof j.data.balance_paise === 'number') {
        setNewBalance(j.data.balance_paise)
        onCredited?.(j.data.balance_paise)
      }
    } else if (j.ok) setState('sent')
    else {
      setState('error')
    ...
    <div className="sheet" role="dialog" aria-label="Recharge">
      ...
      {!upi.id ? (
        ...
        <div className="notice" style={{ borderColor: 'var(--ok)' }}>
          {inr(amount * 100)} added. Your balance is now {inr(newBalance ?? balance + amount * 100)}. A receipt is on its way to your email.
          <p style={{ margin: '10px 0 0' }}><button className="btn sm" onClick={onClose}>Continue</button></p>
        </div>
      ) : state === 'sent' ? (
        <div className="notice" style={{ borderColor: 'var(--ok)' }}>Thanks. We’ll add {inr(amount * 100)} to your balance once the payment is confirmed, and email you when it lands. You can keep using your current balance in the meantime.</div>
      ) : (
        <>
          ...
          <div className="row" style={{ alignItems: 'flex-start', gap: 20 }}>
            ...
            <div className="small" style={{ flex: 1, minWidth: 200 }}>
              ...
              <p className="muted" style={{ marginBottom: 8 }}>Find the 12-digit UTR in your UPI payment details. Payments up to ₹49 are credited instantly.</p>
              <div className="row">
