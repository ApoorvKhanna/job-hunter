// The note. One LLM call grounded in the resume, the posting and the person.
...
- Address the person by first name. Do not mention that you found their email.`
...
export async function POST(req: Request) {
  ...
  if (!body.resume || !body.job) return Response.json({ ok: false, code: 'invalid_params', message: 'Resume and job are required.' })
  const resume = body.resume
  const job = body.job
  const who = body.person?.name ? `${body.person.name} (${body.person.title})` : 'the hiring manager'
  return withVaaya<string>(async (v) => {
    const out = await v.chat(
      [
        { role: 'system', content: SYSTEM },
        {
          role: 'user',
          content: `Write to: ${who} at ${job.company}\nRole: ${job.title}\nPosting URL: ${job.url}\n\nPosting:\n${job.description.slice(0, 3500)}\n\nCandidate resume:\n${resume.slice(0, 8000)}`,
        },
      ],
      450,
    )
    if (!out.ok) return out
    return { ok: true, data: out.data.trim(), chargedCents: 0, balanceCents: null }
  })
}
