...
import { extractJson, paidStep, readJson } from '@/lib/route'
...
{"headline": string (one line, who this person is),
 ...
 "country_code": string (ISO 3166 two-letter code of where they live or want to work; "IN" if unclear),
...
export async function POST(req: Request) {
  ...
  if (text.length < 80) return Response.json({ ok: false, code: 'invalid', message: 'Paste at least a few lines of your resume.' })
  return paidStep<Profile>('parse', async () => {
    ...
    if (!p || !Array.isArray(p.titles) || p.titles.length === 0) {
      return { ok: false, code: 'parse_failed', message: 'Could not read that resume. Try pasting plain text.' }
    }
    return {
      ok: true,
      data: {
        headline: String(p.headline ?? ''),
        titles: p.titles.slice(0, 5).map(String),
        seniority: p.seniority ?? null,
        country_code: (p.country_code || 'IN').toUpperCase().slice(0, 2),
